...
public class AnnotationConfigApplicationContextFactory implements ApplicationContextFactory<AnnotationConfigApplicationContext> {
    private final Class<?>[] configClasses;
    private final Map<String, Object> beans;

    public AnnotationConfigApplicationContextFactory(final Class<?>[] configClasses, final Map<String, Object> beans) {
        this.configClasses = copy(configClasses);
        this.beans = Collections.unmodifiableMap(new HashMap<>(beans));
    }

    @Override
    public AnnotationConfigApplicationContext newInstance() {
        final DefaultListableBeanFactory factory = createBeanFactoryWithSingletons();

        final AnnotationConfigApplicationContext applicationContext = new AnnotationConfigApplicationContext(factory);
